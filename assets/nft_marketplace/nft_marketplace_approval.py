...
class NFTMarketplaceASC1:
    ...
    def make_sell_offer(self, sell_price):
        ...
        app_is_active = Or(
            App.globalGet(self.Variables.app_state) == self.AppState.active,
    ...
    def stop_sell_offer(self):
        ...
        can_stop_selling = And(valid_number_of_transactions, valid_caller, app_is_initialized)

        update_state = Seq([App.globalPut(self.Variables.app_state, self.AppState.active), Return(Int(1))])

        return If(can_stop_selling).Then(update_state).Else(Return(Int(0)))


if __name__ == "__main__":
    print(compileTeal(NFTMarketplaceASC1().application_start(), Mode.Application, version=4))
